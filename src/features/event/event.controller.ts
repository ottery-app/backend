...
export class EventController {
  ...
  async create(
    ...
    try {
      ...
      const volIds = await this.formFieldService.createMany(
        createEventDto.volenteerSignUp,
      );
      const atenIds = await this.formFieldService.createMany(
  ...
  async getVolenteerSignup(@Param('id') id: id) {
    try {
  ...
  async getVolenteerStatus(
    ...
    try {
      const userId = sesh.userId;
      const volenteers = await (
        await this.eventService.findOneById(id)
      ).volenteers;
  ...
  async getAttendeeSignup(@Param('id') id: id) {
    try {
      const event = await this.eventService.findOneById(id);
  ...
  async signupVolenteers(@Param('id') id: id, @Body() ids: id[]) {
    try {
  ...
  async signupAttendees(@Param('id') id: id, @Body() ids: id[]) {
    try {
